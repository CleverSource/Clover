...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name)
{
    ...
    app_info.pEngineName = "Clover Engine";
    ...
    CDEBUG("Vulkan Instance created");
    ...
    if (!vulkan_object_shader_create(&context, &context.object_shader)) {
        CERROR("Error loading built-in basic_lighting shader");
        return false;
    }
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time)
{
    ...
    if (context.recreating_swapchain) {
        ...
        if (!vulkan_result_is_success(result)) {
            CERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
        CINFO("Recreating swapchain, aborting");
        return false;
    }
...
i32 find_memory_index(u32 type_filter, u32 propery_flags)
{
    VkPhysicalDeviceMemoryProperties memory_properties;
    vkGetPhysicalDeviceMemoryProperties(context.device.physical_device, &memory_properties);

    for (u32 i = 0; i < memory_properties.memoryTypeCount; i++) {
        if (type_filter & (1 << i) && (memory_properties.memoryTypes[i].propertyFlags & propery_flags) == propery_flags) {
            return i;
        }
    }
...
{
    if (context.recreating_swapchain) {
        CDEBUG("recreate_swapchain called when already recreating. Aborting");
        return false;
    }

    if (context.framebuffer_width == 0 || context.framebuffer_height == 0) {
        CDEBUG("recreate_swapchain called when window is < 1 in a dimension. Aborting");
        return false;
    }
