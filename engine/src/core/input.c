...
void input_update(f64 delta_time)
{
    ...
    ccopy_memory(&state_ptr->keyboard_previous, &state_ptr->keyboard_current, sizeof(keyboard_state));
    ccopy_memory(&state_ptr->mouse_previous, &state_ptr->mouse_current, sizeof(mouse_state));
}
...
void input_process_key(keys key, b8 pressed)
{
    if (state_ptr && state_ptr->keyboard_current.keys[key] != pressed) {
        ...
        if (key == KEY_LALT) {
            CINFO("Left alt %s", pressed ? "pressed" : "released");
        } else if (key == KEY_RALT) {
            CINFO("Right alt %s", pressed ? "pressed" : "released");
        }
        
        if (key == KEY_LCONTROL) {
            CINFO("Left control %s", pressed ? "pressed" : "released");
        } else if (key == KEY_RCONTROL) {
            CINFO("Right control %s", pressed ? "pressed" : "released");
        }
        
        if (key == KEY_LSHIFT) {
            CINFO("Left shift %s", pressed ? "pressed" : "released");
        } else if (key == KEY_RSHIFT) {
            CINFO("Right shift %s", pressed ? "pressed" : "released");
        }
