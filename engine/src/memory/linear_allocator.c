...
void* linear_allocator_allocate(linear_allocator* allocator, u64 size)
{
    if (allocator && allocator->memory) {
        if (allocator->allocated + size > allocator->total_size) {
            ...
            CERROR("linear_allocator_allocate - Tried to allocate %lluB, only %lluB remaining", size, remaining);
            return 0;
        }
    ...
    CERROR("linear_allocator_allocate - provided allocator not initialized");
    return 0;
}
