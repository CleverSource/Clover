...
void test_manager_run_tests()
{
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        if (result == true) {
            ...
            CWARN("[SKIPPED]: %s", tests[i].desc);
            skipped++;
        } else {
            CERROR("[FAILED]: %s", tests[i].desc);
            failed++;
        }
        char status[20];
        string_format(status, failed ? "*** %d FAILED ***" : "SUCCESS", failed);
        clock_update(&total_time);
        CINFO("Executed %d of %d (skipped %d) %s (%.6f sec / %.6f sec total)", i + 1, count, skipped, status, test_time.elapsed, total_time.elapsed);
    }

    clock_stop(&total_time);

    CINFO("Results: %d passed, %d failed, %d skipped", passed, failed, skipped);
}